class ScoreBoard extends ScoreBoardDeathMatch;
...
var     localized   string      scText,fpText,huskText,sirenText;
...
simulated event UpdateScoreBoard(Canvas Canvas)
{
	...
	local float deathsXL, KillsXL, netXL,HealthXL, MaxNamePos, KillWidthX, HealthWidthX, ScoreXPos, ScoreXL;
    local float scXL,scWidthx,scXPos,fpXL,fpWidthx,fpXPos,huskXL,huskWidthx,huskXPos,sirenXL,sirenWidthx,sirenXPos;
	local bool bNameFontReduction;
	local Material VeterancyBox, StarMaterial;
	local int TempLevel;
	local KFPlayerReplicationInfo KFPRI;
	local KFSSPlayerReplicationInfo SSKFPRI;
	local float CashX;
    ...
    scXPos = BoxXPos + 0.60 * BoxWidth;
    fpXPos = BoxXPos + 0.70 * BoxWidth;
    huskXPos = BoxXPos + 0.80 * BoxWidth;
    sirenXPos = BoxXPos + 0.90 * BoxWidth;
	HealthXpos = BoxXPos + 1.0 * BoxWidth;
	ScoreXPos = BoxXPos + 1.1 * BoxWidth;
	NetXPos = BoxXPos + 1.25 * BoxWidth;
	...
	for (i = 0; i < PlayerCount; i++)
	{
		...
		Canvas.DrawTileStretched(BoxMaterial, BoxWidth*1.2, PlayerBoxSizeY);
	}
    ...
    Canvas.StrLen(sirenText,sirenXL,YL);
    ...
    Canvas.SetPos(scXPos - 0.5f * scXL,TitleYPos);
    Canvas.DrawText(scText,true);

    Canvas.SetPos(fpXPos - 0.5f * fpXL,TitleYPos);
    Canvas.DrawText(fpText,true);

    Canvas.SetPos(huskXPos - 0.5f * huskXL,TitleYPos);
    Canvas.DrawText(huskText,true);

    Canvas.SetPos(sirenXPos - 0.5f * sirenXL,TitleYPos);
    Canvas.DrawText(sirenText,true);
	...
	for (i = 0; i < PlayerCount; i++)
	{
		...
		SSKFPRI = KFSSPlayerReplicationInfo(KFPRI);
		Canvas.DrawColor = HUDClass.default.WhiteColor;
        ...
        //draw sc kills
        Canvas.StrLen(SSKFPRI.scKills, scWidthx, YL);
        Canvas.SetPos(scXPos - 0.5 * scWidthx, (PlayerBoxSizeY + BoxSpaceY) * i + BoxTextOffsetY);
        Canvas.DrawText(SSKFPRI.scKills, true);

        // Draw fp Kill
        Canvas.StrLen(SSKFPRI.fpKills, fpWidthX, YL);
        Canvas.SetPos(fpXPos - 0.5 * fpWidthX, (PlayerBoxSizeY + BoxSpaceY) * i + BoxTextOffsetY);
        Canvas.DrawText(SSKFPRI.fpKills, true);

        // Draw husk Kill
        Canvas.StrLen(SSKFPRI.huskKills, huskWidthX, YL);
        Canvas.SetPos(huskXPos - 0.5 * huskWidthX, (PlayerBoxSizeY + BoxSpaceY) * i + BoxTextOffsetY);
        Canvas.DrawText(SSKFPRI.huskKills, true);

        // Draw siren Kill
        Canvas.StrLen(SSKFPRI.sirenKills, sirenWidthX, YL);
        Canvas.SetPos(sirenXPos - 0.5 * sirenWidthX, (PlayerBoxSizeY + BoxSpaceY) * i + BoxTextOffsetY);
        Canvas.DrawText(SSKFPRI.sirenKills, true);
		// draw cash
